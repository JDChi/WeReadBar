...
- 🔔 **未阅读提醒** —— 默认开启；连续多天未阅读时提醒，天数可在设置中调整
- ⬆️ **应用内更新** —— 每日检查新版，也可从右键菜单手动检查；确认后下载并重启安装
- 🌍 **三语支持** — English / 简体中文 / 繁體中文（跟随系统语言）
- 🔐 **本地保存** —— API 令牌仅保存在本机的应用偏好设置中
- ⚙️ **统一设置** —— 在主面板或右键菜单中管理 API 令牌和阅读提醒
- 🚫 **纯菜单栏体验** —— 没有 Dock 图标；左键查看数据，右键可刷新、检查更新或打开设置
...
- **网络**：WeReadBar 仅访问 `i.weread.qq.com`（微信读书官方 API 网关）和 GitHub Releases（检查及下载应用更新）。无埋点、无遥测。
...
<details>
<summary><b>怎样更新应用？</b></summary>

WeReadBar 每天最多检查一次更新。也可以右键点击菜单栏书本图标，选择「检查更新…」。发现新版后，点击「更新并重启」才会下载和安装；它不会在后台自行下载或重启。
</details>
